...
```tsx
<input use:validate={[minLength(3), isUsernameAvailable]} />
```
...
```tsx
<input use:validate={[isRequired, needsConfirmation() && mustMatchPassword]} />
```
...
<div use:validate={[planChosen]} data-name='plan' />;
```
